...
if __name__ == "__main__":
    ...
    init_worker_ = partial(init_worker, args)
    p = Pool(args.ncpu, initializer=init_worker_)
    print(f"Initialized multiprocessing pool with {args.ncpu} processes")

    # run the decoding with multiprocessing
    # TODO: no speed up over single process. need to refactor to try another method following Hogwild, see link below
    # https://pytorch.org/docs/stable/notes/multiprocessing.html
    trees = []
    decode_one_tree_ = partial(
        decode_one_tree,
        f_act=f_act, f_rt1=f_rt1, f_rt2=f_rt2, f_rxn=f_rxn,
    ...
    for tree in tqdm(
            p.imap(decode_one_tree_, target_smis, chunksize=1),
