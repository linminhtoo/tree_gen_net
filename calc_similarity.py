...
def tanimoto_similarity(smi_a, smi_b, radius, fp_size):
    ...
    return DataStructs.FingerprintSimilarity(fp_a, fp_b,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_decoded_trees", type=Path, default="data/decoded_trees.pickle")
    ...
    tanimoto_similarity_ = partial(tanimoto_similarity, radius=args.radius, fp_size=args.fp_size)
    simils = []
    for simil in tqdm(
            p.starmap(tanimoto_similarity_, zip(target_smis, decoded_smis)),
            total=len(decoded_smis), desc="calculating tanimoto similarities"
        ):
        simils.append(simil)

    simils = np.array(simils)
    print(f"average tanimoto similarity (including failed decoding): {simils.mean():.4f} (+-{simils.std():.4f})")

    success_idxs = (decoded_smis != None)
    simils = simils[success_idxs]
    print(f"average tanimoto similarity (excluding failed decoding): {simils.mean():.4f} (+-{simils.std():.4f})")
