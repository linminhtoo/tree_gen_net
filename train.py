    ...
    for epoch in range(args.epochs):
        ...
        for data in train_loader:
            ...
            if args.model == "f_act" or args.model == "f_rxn":
                ...
                pred_idxs, _ = knn_search_batch(y_pred.detach().cpu().numpy(), index_all_mols, k=1)
        ...
        with torch.no_grad():
            ...
            for data in val_loader:
                ...
                if args.model == "f_act" or args.model == "f_rxn":
                    ...
                    val_correct += torch.sum(torch.eq(pred_idxs, rct_idxs), dim=0).item()

                if not args.logging:
                    val_loader.set_description(f"validating: loss={val_loss / val_seen:.4f}, top-1 acc={val_correct / val_seen:.4f}")
                    val_loader.refresh()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--path_states_train", type=Path, default="data/states_f_act_train.npz")
    parser.add_argument("--path_states_val", type=Path, default="data/states_f_act_val.npz")
    parser.add_argument("--path_steps_train", type=Path, default="data/steps_f_act_train.npz")
    parser.add_argument("--path_steps_val", type=Path, default="data/steps_f_act_val.npz")
    ...
    parser.add_argument("--bs_eval", type=int, default=96, help="batch size, validation")
    parser.add_argument("--input_fp_dim", type=int, default=4096, help="dim of each input fingerprint into network")
    parser.add_argument("--output_fp_dim", type=int, default=256, help="dim of each output fingerprint from network")
    parser.add_argument("--hidden_sizes", type=str, default="1000,1200,3000,3000",
                        help="network hidden_sizes, separated by commas")
    parser.add_argument("--dropout", type=float, default=0.1, help="dropout value")
    # training arguments
    parser.add_argument("--path_checkpoint", type=Path, help="path to store model checkpoints")
